...
class BasicDhcpClient(DhcpClient, dbus.service.Object):
	...
	def sendDhcpDiscover(self):
		...
		print("Sending " + dhcpTypeToName(dhcp_discover_type))
		self._request_sent = False
	...
	def handleDhcpOffer(self, res):
		...
		server_identifier = ipv4(dhcp_offer.GetOption('server_identifier'))
		chaddr = hwmac(dhcp_offer.GetOption('chaddr')[:6])
		yiaddr = ipv4(dhcp_offer.GetOption('yiaddr'))
		self._last_ipaddress = ipv4(dhcp_offer.GetOption('yiaddr'))
		print("We've been given IP: " + str(dhcp_offer.GetOption('yiaddr')))
		self._last_netmask = ipv4(dhcp_offer.GetOption('subnet_mask'))
		print("We've been given netmask: " + str(dhcp_offer.GetOption('subnet_mask')))
		self._last_defaultgw =  ipv4(dhcp_offer.GetOption('router'))
		print("We've been given gw: " + str(dhcp_offer.GetOption('router')))# Router is of type ipv4+ so we could get more than one router IPv4 address... only consider the first one here
		...
		dhcp_request.SetOption('ciaddr',yiaddr.list())
		dhcp_request.SetOption('siaddr',server_identifier.list())
		...
		print("Sending " + dhcpTypeToName(dhcp_request_type))
		self._request_sent = True
		self.SendDhcpPacketTo(dhcp_request, '255.255.255.255', self._server_port)
